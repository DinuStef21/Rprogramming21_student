...
```{r}
ggplot() +
  geom_point(data = penguins, aes (x = flipper_length_mm, y = bill_length_mm, shape = sex, color = species)) + labs(title = "Gay Penguins", subtitle = "Plot by Dinu Stefanescu", caption = "Source: Penguins", x = "Flipper Lentgh", y = "Bill Length")
```
...
```{r}
ggplot() +
  geom_boxplot(data = penguins, aes(x = species, y = body_mass_g, fill = sex)) + labs(y = "body mass in grams") + facet_wrap(~ island)
...
```{r}
ggplot() +
  geom_boxplot(data = penguins, aes(x = species, y = body_mass_g, fill = sex)) + labs(y = "body mass in grams") + scale_fill_manual(values = c(male = "#DA020E", female = "mediumblue")) + facet_wrap(~ island)
