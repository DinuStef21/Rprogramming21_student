...
```{r}
merged_dataframe2 <- full_join(data1, data2)
merged_dataframe2
...
```{r}
merged_dataframe3 <- inner_join(data1, data2)
merged_dataframe3
...
```{r}
merged_dataframe4 <- left_join(data1, data2)
merged_dataframe4


merged_dataframe5 <- left_join(data2, data1)
merged_dataframe5
...
```{r}
merged_dataframe6 <- right_join(data1, data2)
merged_dataframe6


merged_dataframe7 <- right_join(data2, data1)
merged_dataframe7
...
```{r}
data12345678910 <- cbind(data1, data2a)
...
ibrary(readxl)
OECD_Health_Statistics_2020_Frequently_Requested_Data <- read_excel("Desktop/Rprogramming21_student/Lecture_3/sourcedata/OECD-Health-Statistics-2020-Frequently-Requested-Data.xls", 
...
df1 <- read_xls("~/Desktop/Rprogramming21_student/Lecture_3/sourcedata/OECD-Health-Statistics-2020-Frequently-Requested-Data.xls", sheet = 3, skip = 3)
...
```{r}
ggplot(df2, aes(x = Year, y = Expenditure, group = Country, color = Country)) +
  geom_line() +
  labs(x = "", y = "Expenditure per capita in PPP") +
...
```{r}
ggplot(df5, aes(x = reorder(Country, desc(Growth)), y = Growth, color = Country, fill = Country)) +
  geom_col() +
  labs(x = "", legend.title = element_blank())
